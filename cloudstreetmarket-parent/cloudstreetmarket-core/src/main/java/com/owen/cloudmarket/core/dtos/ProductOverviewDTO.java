...
public class ProductOverviewDTO implements Serializable
{
	...
	public ProductOverviewDTO(String name, String code, String market,
	...
	{
		return new ProductOverviewDTO(product.getName(), product.getCode(),
				(product.getMarket() != null) ? product.getMarket().getName()
						: null, product.getCurrency(),
				product.getDailyLatestValue(), product.getDailyLatestChange(),
				product.getDailyLatestChangePercent(),
				product.getPreviousClose(), product.getHigh(), product.getLow());
	}
}