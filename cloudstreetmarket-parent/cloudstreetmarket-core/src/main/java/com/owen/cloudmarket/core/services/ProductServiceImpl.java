...
import com.owen.cloudmarket.core.specifications.ProductSpecifications;

@Service
public class ProductServiceImpl<T extends Product, U extends ProductOverviewDTO>
		implements IProductService<T, U>
{
	...
	@Override
	public Page<U> getProductsOverview(String startWith, Specification<T> spec,
			Pageable pageable)
	{
		...
		if (StringUtils.isNotBlank(startWith))
		{
			spec = Specifications.where(spec).and(
					new ProductSpecifications<T>().nameStartsWith(startWith));
		}

		Page<T> products = productRepository.findAll(spec, pageable);

		return new PageImpl<>(transformTypedProductsInDTOs(products), pageable,
